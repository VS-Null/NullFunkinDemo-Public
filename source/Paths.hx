...
class Paths
{
	...
	// new RAM cleaner by M&M & coauthored by mcagabe19
	...
	static public function getTextFromFile(key:String, ?ignoreMods:Bool = false):String
	{
		...
		#end
		return #if !html5 Assets.getText(getPath(key, TEXT)); #else OpenFlAssets.getText(getPath(key, TEXT)); #end
	}
	...
	public static function returnGraphic(key:String, ?library:String, ?gpurender:Bool = false)
	{
				...
				if (gpurender)
				{
					switch (ClientPrefs.render)
					{
						case 1:
							...
							bitmap.disposeImage();
							bitmap = null;
							newGraphic = FlxGraphic.fromBitmapData(BitmapData.fromTexture(texture), false, modKey);
						case 2:
							...
							newGraphic = FlxGraphic.fromBitmapData(BitmapData.fromTexture(texture), false, modKey);
						default:
							newGraphic = FlxGraphic.fromBitmapData(bitmap, false, modKey);
					}
				}
				else
					newGraphic = FlxGraphic.fromBitmapData(bitmap, false, modKey);
		...
		trace('$key image returning to null 😭');
		return null;
	}
