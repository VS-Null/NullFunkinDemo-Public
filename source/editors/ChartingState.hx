...
class ChartingState extends MusicBeatState
{
	...
	var eventStuff:Array<Dynamic> = [
		...
		['Set beat interval', "Value 1: from 0 to 1 and etc\nValue 2: Nothing"], // ['Shakey Notes', "Value 1: X Axis\nValue 2: Y Axis\n use , for the player"],
		// ['Bump Notes', "Value 1: Amount\nValue 2: Delay"]
		['HUD opacity', "Value 1: Alpha\nValue 2: Duration (optional)"],
		['StrumLine opacity', "Value 1: Alpha\nValue 2: Duration (optional)"],
		['Zoom in', "Value 1: Zoom\nValue 2: Duration (optional)"]
	];
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var directories:Array<String> = [
			...
			SUtil.getStorageDirectory() + Paths.getPreloadPath('characters/')
		];
		...
		var directories:Array<String> = [
			...
			SUtil.getStorageDirectory() + Paths.getPreloadPath('stages/')
		];
		for (mod in Paths.getGlobalMods())
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			if (FlxG.keys.justPressed.ESCAPE #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonB.justPressed #end)
			{
				autosaveSong();
				LoadingState.loadAndSwitchState(new editors.EditorPlayState(sectionStartTime()));
			}
			if (FlxG.keys.justPressed.ENTER #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonA.justPressed #end)
			{
				autosaveSong();
			...
			if (FlxG.keys.justPressed.Z #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonZ.justPressed #end && FlxG.keys.pressed.CONTROL)
			{
				undo();
			}

			if (FlxG.keys.justPressed.Z #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonZ.justPressed #end && curZoom > 0
				&& !FlxG.keys.pressed.CONTROL)
			{
				--curZoom;
				updateZoom();
			}
			if (FlxG.keys.justPressed.X #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonC.justPressed #end && curZoom < zoomList.length - 1)
			{
				curZoom++;
				updateZoom();
			}
			...
			if (FlxG.keys.justPressed.SPACE #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonX.justPressed #end)
			{
			...
			if (FlxG.keys.pressed.W || FlxG.keys.pressed.S #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonUp.pressed
				|| FlxG.save.data.mobileControls && virtualPad.buttonDown.pressed #end)
			{
				FlxG.sound.music.pause();
				...
				if (FlxG.keys.pressed.CONTROL)
					holdingShift = 0.25;
				else if (FlxG.keys.pressed.SHIFT #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonY.pressed #end)
					holdingShift = 4;

				var daTime:Float = 700 * FlxG.elapsed * holdingShift;

				if (FlxG.keys.pressed.W #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonUp.pressed #end)
				{
			...
			if (!vortex)
			{
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls
					&& virtualPad.buttonUp.justPressed || virtualPad.buttonDown.justPressed #end)
				{
					FlxG.sound.music.pause();
					updateCurStep();
					var time:Float = FlxG.sound.music.time;
					...
					var snap:Float = quantization / 4;
					var increase:Float = 1 / snap;
					if (FlxG.keys.pressed.UP #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonUp.pressed #end)
					{
			...
			if (FlxG.keys.pressed.SHIFT #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonY.pressed #end)
			{
				style = 3;
			}
			...
			if (!blockInput)
			{
				if (FlxG.keys.justPressed.RIGHT #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonRight.justPressed #end)
				{
					curQuant++;
				...
				if (FlxG.keys.justPressed.LEFT #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonLeft.justPressed #end)
				{
					curQuant--;
					if (curQuant < 0)
			...
			if (FlxG.keys.justPressed.D #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonRight.justPressed #end)
				changeSection(curSec + shiftThing);
			if (FlxG.keys.justPressed.A #if (mobileC || mobileCweb) || FlxG.save.data.mobileControls && virtualPad.buttonLeft.justPressed #end)
			{
				if (curSec <= 0)
				{
	...
	function loadHealthIconFromCharacter(char:String)
	{
		...
		#end
		{
			path = SUtil.getStorageDirectory()
				+ Paths.getPreloadPath('characters/' + Character.DEFAULT_CHARACTER +
